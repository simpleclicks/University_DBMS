...
public class CourseDaoImpl implements IDao {
	...
	public String find(Object object) {
...

		{
			...
			// Add some code if the results do not come up in any one of the
			// above searches.
	...
	public String findAll() {
		...
		String result = "";
		try {
			String query = "SELECT c.courseId,c.courseName, c.location,ct.day,ct.time FROM course AS c LEFT JOIN coursetiming AS ct ON ct.courseId = c.courseId ";
	...
	public String update(Object object) {
		...
		try {
			String query = "Update course set courseName = '"
					+ c.getCourseName() + "', location = '" + c.getLocation()
					+ "' where courseId='" + c.getCourseId() + "'";
			res = stmt.executeUpdate(query);
			query = "Update coursetiming set day = '" + c.getDays()
					+ "', time = '" + c.getTimings() + "' where courseId='"
					+ c.getCourseId() + "'";
			res = stmt.executeUpdate(query);
		} catch (SQLException e) {
			e.printStackTrace();
